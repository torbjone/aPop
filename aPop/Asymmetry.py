...
def Asymmetry(params):
    ...
    if rank == 0:
        ...
        task = 0
        num_cells = 1
        num_tasks = len(fractions) * len(mus) * len(dists) * num_cells
                ...
                for mu in mus:
                    for cell_idx in xrange(num_cells):
                        ...
                        while not sent:
                            ...
                            tag = status.Get_tag()
                            if tag == tags.READY:
                                comm.send([fraction, channel_dist, mu, cell_idx], dest=source, tag=tags.START)
        ...
        while True:
            ...
            [fraction, channel_dist, mu, cell_idx] = comm.recv(source=0, tag=MPI.ANY_TAG, status=status)
            tag = status.Get_tag()
            if tag == tags.START:
                # Do the work here
...
if __name__ == '__main__':
    from param_dicts import asymmetry_params
    if len(sys.argv) == 5:
        ns = NeuralSimulation(**asymmetry_params)
        args = [float(sys.argv[1]), float(sys.argv[2]), sys.argv[3], int(sys.argv[4])] #mu, fraction, channel_dist, cell_idx
        ns.run_asymmetry_simulation(*args)
